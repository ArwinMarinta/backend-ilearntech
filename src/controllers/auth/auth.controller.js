...
module.exports = {
    ...
    try {
      ...
      if (emailSent) {
        ...
        return res
          ...
          .json(
            utils.apiSuccess(
              "Pendaftaran akun berhasil. Periksa email masuk untuk kode verifikasi Otp",
              data
            )
          );
      } else {
  ...
  verifyUser: async (req, res) => {
    try {
      ...
      if (!checkOtp)
        return res
          .status(404)
          .json(utils.apiError("Otp tidak ditemukan. Silahkan kirim ulang kembali"));
  ...
  requestResetPassword: async (req, res) => {
    try {
      ...
      if (user.resetToken != null)
        return res
          .status(500)
          .json(utils.apiError("Link untuk reset password sudah dikirim ke email anda"));
      ...
      return res
        .status(200)
        .json(utils.apiSuccess("Periksa email anda untuk link reset password"));
    } catch (error) {
      console.log(error);
      return res.status(500).json(utils.apiError("Kesalahan pada internal server"));
    }
  ...
  resetPassword: async (req, res) => {
    try {
      ...
      if (!user)
        return res.status(500).json(utils.apiError("Reset password token invalid"));
  ...
  resendOtp: async (req, res) => {
    try {
      ...
      if (emailSent) {
        return res
          .status(200)
          .json(utils.apiSuccess("Otp berhasil dikirim ulang. Periksa email masuk"));
      }
    } catch (error) {
      console.log(error);
      return res.status(500).json(utils.apiError("Kesalahan pada internal server"));
  ...
  updateProfilePhoto: async (req, res) => {
    try {
      ...
      if (typeof photoProfile === "undefined")
        return res.status(400).json(utils.apiError("Gambar tidak boleh kosong"));

      if (!allowedMimes.includes(photoProfile.mimetype))
        return res
          .status(400)
          .json(utils.apiError("Harus bertipe gambar (.png, .jpeg, .jpg, .webp)"));

      if (photoProfile.size / (1024 * 1024) > allowedSizeMb)
        return res.status(400).json(utils.apiError("Gambar tidak boleh lebih dari 2mb"));
