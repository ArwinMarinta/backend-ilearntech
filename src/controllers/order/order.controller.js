...
const userLearningProgress = require("../../utils/user-learning-progress");
...
module.exports = {
  getOrders: async (req, res) => {
    try {
      ...
      return res.status(200).json(
        utils.apiSuccess("Berhasil mengambil data riwayat semua order", data, {
  ...
  getOrderHistoryById: async (req, res) => {
    try {
      ...
      if (!historyOrders)
        return res.status(404).json(utils.apiError("Riwayat order tidak ditemukan"));

      return res
        .status(200)
        .json(
          utils.apiSuccess("Berhasil mengambil riwayat order berdasarkan id user", data)
        );
    } catch (error) {
      console.log(error);
      return res.status(500).json(utils.apiError("Kesalahan pada Internal Server "));
  ...
  enrollFree: async (req, res) => {
    try {
      ...
      if (type !== "Free")
        return res
          .status(403)
          .json(utils.apiError("Ini adalah untuk proses ambil course gratis"));
      ...
      if (!existUserCourse) {
        ...
        contents.forEach(async (item, index, array) => {
          await userLearningProgress.createUserLearningProgress(
            item.id,
            orderFreeCourse.id
          );
        });
        ...
        return res
          .status(201)
          .json(utils.apiSuccess("Berhasil Mengambil Kelas", orderFreeCourse));
  ...
  orderPremium: async (req, res) => {
    ...
    if (!paymentMethod)
      return res.status(422).json(utils.apiError("paymentMethod tidak boleh kosong"));
    ...
    if (checkOrder)
      return res
        .status(409)
        .json(utils.apiError("Anda sudah melakukan order. Selesaikan proses pembayaran"));
    ...
    if (type !== "Premium")
      return res
        .status(403)
        .json(utils.apiError("Ini adalah untuk proses ambil course premium"));
    ...
    if (!existUserCourse) {
      ...
      await notification.createNotification(
        "Proses Pembayaran",
        null,
        "Silahkan lakukan proses pembayaran untuk course yang telah diorder",
        userId
      );
      ...
      return res
        ...
        .json(
          utils.apiError(
            "Lakukan proses pembayaran untuk menyelesaikan pembelian course ini",
            order
          )
        );
    } else {
    ...
    try {
      ...
      if (confimrOrder) {
        ...
        await notification.createNotification(
          "Pembayaran Berhasil",
          null,
          "Pembayaran kelas premium berhasil dilakukan. Anda bisa mengakses kelas ini sekarang juga!",
          userId
        );

        return res
          .status(201)
          .json(utils.apiSuccess("Konfirmasi pembelian berhasil", orderFreeCourse));
      }
    } catch (error) {
      console.log(error);
      return res.status(500).json(utils.apiError("Kesalahan pada Internal Server "));
