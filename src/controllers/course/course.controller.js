...
module.exports = {
  getCourses: async (req, res) => {
    try {
      ...
      whereCondition = await filter.course.filterWhereCondition(
        whereCondition,
        search,
        category,
        level,
      ...
      const message = await filter.message.filterMessage({
        search,
        category,
        level,
        type,
  ...
  getCourseById: async (req, res) => {
    try {
        ...
        if (userCourse) {
          ...
          data.modules.forEach((module) => {
            module.contents.forEach((content) => {
              const status = userLearningProgress.find(
                (progress) => progress.contentId === content.contentId
              );
      ...
      return res
        .status(200)
        .json(utils.apiSuccess("Data course berdasarkan id berhasil diambil", data));
    } catch (error) {
      console.log(error);
      return res.status(500).json(utils.apiError("Kesalahan pada internal server"));
    }
  ...
  createCourse: async (req, res) => {
    try {
      ...
      if (checkTitle)
        return res.status(409).json(utils.apiError("Judul course sudah terdaftar"));
      ...
      if (!checkLevel)
        return res.status(404).json(utils.apiError("Level tidak ditemukan"));
      ...
      if (!checkInstructor)
        return res.status(404).json(utils.apiError("Instructor tidak ditemukan"));
      ...
      if (typeof courseImage === "undefined")
        return res.status(422).json(utils.apiError("Gambar kelas tidak boleh kosong"));

      if (!allowedMimes.includes(courseImage.mimetype))
        return res.status(409).json(utils.apiError("Format gambar tidak diperbolehkan"));

      if (courseImage.size / (1024 * 1024) > allowedSizeMb)
        return res
          .status(409)
      ...
      const cattegoryAbbrevation = categorySlug
        .split("-")
  ...
  updateCourse: async (req, res) => {
    try {
      ...
      if (!checkCourse)
        return res.status(404).json(utils.apiError("Course tidak ditemukkan"));
      ...
      if (!checkCategory)
        return res.status(404).json(utils.apiError("Kategori course tidak ditemukan"));
      ...
      if (!checkType)
        return res.status(404).json(utils.apiError("Tipe course tidak ditemukan"));
      ...
      if (!checkLevel)
        return res.status(404).json(utils.apiError("Level course tidak ditemukan"));
  ...
  putPromoOnCourse: async (req, res) => {
    try {
      ...
      if (!checkPromo)
        return res.status(404).json(utils.apiError("Promo tidak ditemukkan"));
  ...
  cancelPromoOnCourse: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json(utils.apiSuccess("Behasil menghapus promo pada course"));
    } catch (error) {
      console.log(error);
      return res.status(500).json(utils.apiError("Kesalahan pada internal server"));
    }
  ...
  unpublishCourse: async (req, res) => {
    try {
      ...
      if (publish === "false") {
        ...
        return res
          .status(201)
          .json(utils.apiSuccess("Berhasil unpublish course", course));
      }
    } catch (error) {
      console.log(error);
      return res.status(500).json(utils.apiError("Kesalahan pada internal server"));
