...
module.exports = {
  read: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json(utils.apiSuccess("Berhasil mengambil semua data instructor", data));
    } catch (error) {
      console.log(error);
  ...
  readById: async (req, res) => {
    ...
    try {
      ...
      return res
        ...
        .json(
          utils.apiSuccess(
            "Berhasil mengambil data instructor berdasarkan id",
            instructor
          )
        );
    } catch (error) {
  ...
  update: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json(utils.apiSuccess("Data instruktur berhasil diperbarui", data));
    } catch (error) {
      console.log(error);
      return res.status(500).json(utils.apiError("Kesalahan pada internal server"));
    }
