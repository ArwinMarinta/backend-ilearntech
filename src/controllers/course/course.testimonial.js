...
module.exports = {
  createTestimonial: async (req, res) => {
    try {
      ...
      if (!checkCourse)
        return res.status(404).json(utils.apiError("Kelas tidak ditemukan"));
      ...
      if (checkTestimonial)
        return res
          .status(409)
          .json(
            utils.apiError("Anda sudah memberikan testimoni dan rating untuk kelas ini")
          );
      ...
      return res
        ...
        .json(
          utils.apiSuccess(
            "Berhasil memberikan testimoni dan rating untuk kelas ini",
            createTestimoni
          )
        );
    } catch (error) {
  ...
  readTestimonialByCourseId: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json(
          utils.apiSuccess("Behasil mengambil data testimonial berdasarkan course", data)
        );
    } catch (error) {
      console.log(error);
      return res.status(500).json(utils.apiError("Kesalahan pada internal server"));
  ...
  updateTestimonial: async (req, res) => {
    try {
      ...
      if (!checkTestimonial)
        return res.status(404).json(utils.apiError("Testimonial tidak ditemukkan"));
      ...
      return res
        ...
        .json(
          utils.apiSuccess(
            "Berhasil mengubah testimoni dan rating untuk kelas ini",
            createTestimoni
          )
        );
    } catch (error) {
