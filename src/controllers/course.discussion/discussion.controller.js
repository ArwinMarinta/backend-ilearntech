...
module.exports = {
  createDiscussionByIdCourse: async (req, res) => {
    try {
      ...
      if (req.file) {
        ...
        if (!allowedMimes.includes(photoDiscussion.mimetype))
          return res
            .status(409)
            .json(utils.apiError("Format gambar tidak diperbolehkan"));

        if (photoDiscussion.size / (1024 * 1024) > allowedSizeMb)
          return res
            .status(409)
      ...
      return res
        .status(201)
        .json(utils.apiSuccess("Berhasil membuat diskusi", discussion));
    } catch (error) {
      console.log(error);
      return res.status(500).json(utils.apiError("Kesalahan pada Internal Server"));
    }
  ...
  updateDiscussionByIdCourse: async (req, res) => {
    try {
      ...
      return res
        .status(201)
        .json(utils.apiSuccess("Berhasil update data diskusi", discussion));
    } catch (error) {
      console.log(error);
      return res.status(500).json(utils.apiError("Kesalahan pada Internal Server"));
    }
